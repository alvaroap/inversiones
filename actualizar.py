...
MIS_TITULOS = {
    ...
    "vanguard": {"ticker": "0P00000RQC.F", "titulos": 130.46}, 
    ...
    for activo in data["activos"]:
        ...
        if MESES_MAP[activo["desdeMes"]] <= num_mes_dest <= MESES_MAP[activo["hastaMes"]]:
            ...
            try:
                ...
                precio_mercado = hist['Close'].tail(1).values[0] if not hist.empty else 0
            except:
                continue # Si falla el robot, salta al siguiente

            valor_total = round(precio_mercado * info["titulos"])
            data["datos"][mes_dest][id_act]["valor"] = valor_total

            if not es_auto: # Solo pide aportación si lo haces tú manual
